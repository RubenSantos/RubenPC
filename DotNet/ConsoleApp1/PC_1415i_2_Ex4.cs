...
namespace Estudo
{
    ...
    ​
	    public class SyncOps {
		    ...
		    public bool CheckDeviceVersion(Services svc,int devId){
			    ...
			    int stoVer = vc.GetStoredVersion(devId);
    ...
    **/
    ...
    class APMAndTAPOps
    {
        ...
        {
            Task<int> devVert = svc.GetDeviceAddressAsync(devId).ContinueWith((_ant) => svc.GetVersionFromDeviceAsync(_ant.Result)).Unwrap();
            Task<int> stoVert = svc.GetStoredVersionAsync(devId);
            return Task<bool>.Factory.ContinueWhenAll(new Task[] { stoVert, devVert }, (_) => stoVert.Result == devVert.Result);
        }
